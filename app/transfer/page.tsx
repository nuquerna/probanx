...
const Transfer: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = (data: IFormInput) => {
    ...
    form.resetFields();
    if (activeAccount)
      setValue("payerAccount", activeAccount.iban, { shouldValidate: false });
  ...
  const balance = useMemo(() => {
    if (activeAccount) {
      if (language === "lt")
        return new Intl.NumberFormat("lt-LT").format(activeAccount.balance);
      if (language === "en")
        return new Intl.NumberFormat("en-US").format(activeAccount.balance);
    }
    return activeAccount?.balance;
  }, [activeAccount, language]);

  const invalidAccount = useMemo(
    () => !activeAccount || activeAccount?.balance < 0.1,
    [activeAccount]
  );
  ...
  return (
    <Flex gap="middle" vertical>
      <Space wrap>
        ...
        {invalidAccount ? (
          <Alert
            message="There is no active account or Your balance is low"
            type="error"
          />
        ) : null}
      </Space>
              ...
              rules={[
                {
                  ...
                  validator: (_rule, value, cb) => {
                    ...
                    if (value.length < 15)
                      cb("IBAN should not be less than 15 symbols");
                    validateIban(value).then((data) =>
                      data ? cb(data) : cb()
                    );
                  },
                },
